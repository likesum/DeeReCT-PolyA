...
MOUSE_MOTIF_VARIANTS = ['AATAAA', 'ATTAAA', 'TATAAA', 'AGTAAA', 'AAGAAA', 'AATATA', 'AATACA', 'CATAAA', 'GATAAA','AATGAA', 'TTTAAA','ACTAAA', 'AATAGA']
...
def motif_data_split(pos_data, pos_labels, neg_data, neg_labels, num_folds, split):
    ...
    train_data = np.concatenate([motif_data[motif]['train_dataset'] for motif in MOUSE_MOTIF_VARIANTS], axis=0)
    valid_data = np.concatenate([motif_data[motif]['valid_dataset'] for motif in MOUSE_MOTIF_VARIANTS], axis=0)
    test_data = np.concatenate([motif_data[motif]['test_dataset'] for motif in MOUSE_MOTIF_VARIANTS], axis=0)
    train_labels = np.concatenate([motif_data[motif]['train_labels'] for motif in MOUSE_MOTIF_VARIANTS], axis=0)
    valid_labels = np.concatenate([motif_data[motif]['valid_labels'] for motif in MOUSE_MOTIF_VARIANTS], axis=0)
    test_labels = np.concatenate([motif_data[motif]['test_labels'] for motif in MOUSE_MOTIF_VARIANTS], axis=0)
    ...
    data['motif_dataset'] = {motif: {} for motif in MOUSE_MOTIF_VARIANTS}
    for motif in MOUSE_MOTIF_VARIANTS:
...
def produce_dataset(num_folds, pos_path, neg_path, split=None):
...
def produce_motif_dataset(num_folds, pos_path, neg_path, split=None):
...
def motif_exclude_data_split(pos_data, pos_labels, neg_data, neg_labels, num_folds, split, excluded_motifs=[]):
    ...
    train_data = np.concatenate([motif_data[motif]['train_dataset'] for motif in MOUSE_MOTIF_VARIANTS if motif not in excluded_motifs], axis=0)
    valid_data = np.concatenate([motif_data[motif]['valid_dataset'] for motif in MOUSE_MOTIF_VARIANTS if motif not in excluded_motifs], axis=0)
    train_labels = np.concatenate([motif_data[motif]['train_labels'] for motif in MOUSE_MOTIF_VARIANTS if motif not in excluded_motifs], axis=0)
    valid_labels = np.concatenate([motif_data[motif]['valid_labels'] for motif in MOUSE_MOTIF_VARIANTS if motif not in excluded_motifs], axis=0)
